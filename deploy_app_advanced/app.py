...
with right_panel:
    if "last_model_input" in st.session_state:
        ...
        if active_mode == "Detailed Mode (Advanced)":
            ...
            st.caption("Ask questions about your Vitamin D result and how to improve it. If API is unavailable, offline guidance is used.")
            ...
            if user_prompt:
                ...
                if not api_key:
                    bot_reply = (
                        ...
                        + get_rule_based_reply(user_prompt, model_input, prediction, status)
                    )
                else:
                    try:
